...
const DownloadCenter: React.FC = () => {
  ...
  const fetchDocs = async () => {
    try {
      setLoading(true);
      const res = await api.get(`/download-docs/category/${category}`);
      if (res.data.success) {
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    try {
      setUploading(true);
      const res = await api.post(`/download-docs`, formData, {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await api.delete(`/download-docs/${id}`);
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full text-center md:text-left">
            ...
            >
              ...
              <p className="text-blue-100/80 text-lg md:text-xl font-medium max-w-xl mx-auto md:mx-0 leading-relaxed">
                Access authenticated {category} documentation and administrative
                forms directly from Baksh Group Gateway.
              </p>
            </motion.div>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 -mt-12 mb-16 relative z-20">
          <div className="flex items-center gap-4 bg-white/80 backdrop-blur-xl p-2 rounded-2xl shadow-sm border border-white/50">
            ...
            <h2 className="text-xl font-bold text-slate-800 pr-6">
              Document Library
            </h2>
          </div>
        ...
        {loading ? (
          <div className="flex flex-col justify-center items-center py-40">
            ...
            <p className="mt-6 text-slate-400 font-semibold tracking-wide uppercase text-xs">
              Accessing Archives...
            </p>
          </div>
        ) : docs.length === 0 ? (
          ...
          >
            ...
            <p className="text-slate-500 text-lg max-w-sm mx-auto leading-relaxed">
              No {category} documents have been published to the hub at this
              time.
            </p>
          </motion.div>
        ) : (
              ...
              {docs.map((doc, index) => (
                ...
                >
                  ...
                  <div className="px-10 pb-10">
                    ...
                    >
                      <span className="font-bold text-slate-700 tracking-wide group-hover:text-white">
                        ACCESS FILE
                      </span>
